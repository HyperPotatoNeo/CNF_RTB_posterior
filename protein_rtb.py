...
class ProteinRTBModel(nn.Module):
    ...
    def forward(
        ...
        for step, _ in enumerate((pbar := tqdm(range(steps)))):
            ...
            posterior_drift = -self.sde.drift(t, x) - (g ** 2) * (self.model_and_shape(t, x) + lp_correction) / self.sde.sigma(t).view(-1, *[1]*len(D))
    ...
    def batched_forward(
        ...
        for i, batch_steps in enumerate(utils.create_batches(steps, traj_batch)):
            ...
            f_posterior = -self.sde.drift(t_, xs) - g ** 2 * (self.model_and_shape(t_[:,0,0,0], xs) + lp_correction) / self.sde.sigma(t_).view(-1, *[1]*len(D))
    ...
    def batched_forward_ddpm(
        ...
        for i, batch_steps in enumerate(utils.create_batches(steps, traj_batch)):
            ...
            lp_correction = self.get_langevin_correction(xs)
            f_posterior = self.sde.drift(t_, xs, -dts) + self.model_and_shape(t_[:,0,0,0], xs) + lp_correction #/ self.sde.sigma(t_).view(-1, *[1]*len(D))
